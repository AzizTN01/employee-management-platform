...
Class Action {
	...
	function save_indem(){
		...
		$sql = "INSERT INTO `indemnité_parent`( `mois`, `total`) VALUES ('$mois','$indemT')";
		$rs = mysqli_query($this->db,$sql);
		//echo("INSERT INTO `indemnité_parent`( `mois`, `total`) VALUES ('$mois','$indemT')");
		$j=0;
		foreach ($Matricule as $mle)  
		{
			$sql = "INSERT INTO indemnite_detaill( mat,base,indem,absence, indemf) VALUES ('$mle','$base[$j]','$indem[$j]','$joursabsent[$j]','$indemf[$j]')";
	...
	function delete_parcel(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM stages_parent where id = '$id'");
		$delete1 = $this->db->query("DELETE FROM stages_details where id = '$id'");
		if($delete&&$delete1){
			return 1;
		}
	}
	function print(){
		extract($_POST);
		$qry = $this->db->query("SELECT * FROM stages_parent as p,stages_details as d WHERE p.id=d.id_parent AND p.id='$id' ");
		$qrymle = $this->db->query("SELECT * FROM stages_parent as p,stages_details as d WHERE p.id=d.id_parent AND p.id='$id' ");
		echo("<h1>matruule:</h1>");
		echo $Mle['N'] ;
		if($qry&&$qrymle){
			return 1;
	...
	function get_report(){
		extract($_POST);
		$data = array();
		$get = $this->db->query("SELECT *,p.id as ID FROM stages_parent as p,stages_details as d WHERE p.id=d.id_parent AND p.actif='1' order by p.id DESC");
